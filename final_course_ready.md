...
| Принцип | Суть | Антипаттерн | Пример нарушения |
|---------|------|-------------|------------------|
| SRP | У класса одна причина для изменения | God Class | Класс OrderService, который и валидирует, и сохраняет, и отправляет email |
| OCP | Класс открыт для расширения, закрыт для изменения | Модификация существующего кода | if (type == "email") вместо strategy pattern |
| LSP | Подтипы не должны нарушать контракт базового типа | Нарушение постусловий | Square extends Rectangle - изменение ширины меняет высоту |
| ISP | Много specialised интерфейсов лучше одного общего | Fat Interface | Интерфейс Worker с методами eat(), code(), sleep() |
| DIP | Зависимость от абстракций, не от конкреций | Жёсткая связность | new MySqlRepository() вместо инъекции Repository |
| DRY | Не повторяйся | Copy-paste | Один и тот же SQL-запрос в 5 разных местах |
| KISS | Простота - залог надёжности | Overengineering | Фабрика фабрик для создания 2 объектов |
| YAGNI | Не пиши то, что не нужно сейчас | "На всякий случай" | Архитектура под "будущие расширения" |
| LoD | Общайся только с ближайшими соседями | Train Wreck | order.getCustomer().getAddress().getCity() |
| SoC | Раздели ответственность между модулями | Смешение слоёв | SQL-запрос прямо в HTML-шаблоне |

#### Метрики графа зависимостей (SDP)

Для каждого компонента/пакета можно вычислить:

- **Instability (I)** = Fan-out / (Fan-in + Fan-out) - мера нестабильности
- **Abstractness (A)** = Na / Nc - доля абстрактных классов/интерфейсов
